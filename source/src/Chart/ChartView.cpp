      ...
      m_markerSignalMapper(new QSignalMapper)
{
    ...
    auto connectSeries = [this](QAbstractSeries *series) {
        connect(static_cast<LineSeries *>(series), &LineSeries::leftClicked,
                this, &ChartView::keepCallout);
        connect(static_cast<LineSeries *>(series), &LineSeries::hovered,
                this, &ChartView::tooltip);
        ...
        foreach (QLegendMarker *marker, this->m_chart->legend()->markers())
        {
            ...
            connect(marker, &QLegendMarker::clicked,
                    m_markerSignalMapper, static_cast<void (QSignalMapper::*)(void)>(&QSignalMapper::map),
                    Qt::UniqueConnection);
            connect(m_markerSignalMapper, static_cast<void (QSignalMapper ::*)(QObject *)>(&QSignalMapper::mappedObject),
                    this, &ChartView::toggleSeriesVisibility,
                    Qt::UniqueConnection);
        }
    };
...
void ChartView::resizeEvent(QResizeEvent *event)
{
    ...
    m_chart->setXAxiTickCount(width() / 100);
    m_chart->setYAxiTickCount(height() / 100);
}
